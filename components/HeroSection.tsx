...
function renderPreviewPanel(data: ProjectData) {
  return (
    <div className="space-y-4">
      <div
        className="rounded-[1.75rem] border border-white/10 bg-white/10 p-6 text-white backdrop-blur-sm"
      ...
      >
        ...
        <div className="mt-6 space-y-3">
          {data.tableRows.slice(0, 3).map((row) => (
            <div key={row.id} className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/10 px-4 py-3">
              <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {data.stats.slice(1, 3).map((stat) => (
          <div key={stat.label} className="rounded-[1.5rem] border border-white/10 bg-white/10 px-5 py-4 text-white backdrop-blur-sm">
...
export function HeroSection({ data }: HeroSectionProps) {
  ...
  return (
    <section id="top" className="px-4 pt-8 md:px-8 lg:px-12">
      <div
        className="mx-auto max-w-7xl overflow-hidden rounded-[2.5rem] px-6 py-14 text-white md:px-10 md:py-20 lg:px-14"
        style={{
          ...
          <div className={centered ? undefined : "max-w-2xl"}>
            ...
            {centered ? (
              <div className="mt-12 grid gap-4 md:grid-cols-4">
                {data.stats.map((stat) => (
                  <div key={stat.label} className="rounded-[1.5rem] border border-white/10 bg-white/10 px-5 py-4 text-left backdrop-blur-sm">
                    <p className="text-xs uppercase tracking-[0.2em] text-white/60">{stat.label}</p>
